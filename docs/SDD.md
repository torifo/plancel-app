...
| 5 | テキスト一次パーサーは **Groq**、二次 Gemini Flash（当初の選定は Llama 3.3 70B。**モデル名は ADR-13 で更新済み** — 引退したため） | 二次を画像経路と同一プロバイダに揃え実装共通化。**無料枠条件は流動的なためデプロイ前に現行クォータを再確認すること**。**2026-07-30 追記**: 「N日前から◯%」型の規定を含むテキストだけ per-input で Gemini 優先(`chainForInput`)。無料境界は原文から上書きする。**2026-08-01 追記**: 通常テキストはGroqだけで完了し、決定的な疑義条件だけGemini査読。Geminiの正確な有効枠はGoogle AI Studioのプロジェクト/モデル表示を正とし、画像経路用の枠を温存する |
...
| 9 | 既存予約の取り込みは「インサート優先・手動は最後の手段」(2026-07-05) | 既に予約を持つ新規ユーザーの主経路は ①予約確認メールの貼り付け/転送パース ②スクショ(LINE経由・§5パイプライン) ③カレンダー取り込み(.ics インポート — Google カレンダー / TimeTree はエクスポート経由で即対応可。API 直接連携は要調査: TimeTree 公開 API は提供終了歴があるため)。手動一括入力はフォールバックとして残すが UI 上は最後に置く。§2「入口を増やしても正規化コードは増やさない」の原則どおり、全て §5 の共通パイプラインに流す |
| 10 | 通知チャネル実装を確定(2026-07-10): LINE = Messaging API push、Email = **Resend** | 無料枠は LINE 月200通 / Resend 約100通/日・3,000通/月(いずれも流動的、デプロイ前に再確認)。Notifier 抽象の背後で Console/LINE/Email を差し替え。LINE webhook は署名検証 + userId 許可リスト、FieldConflict は Quick Reply ワンタップ解決(解決状態は ParseJob.conflicts の絞り込みで永続化、attempts の生履歴は不変)。欠損フィールドは質問を返し再送してもらう(状態を持つ会話フローは v1.x 検討) |

| 11 | Web台帳の「終わった」は**状態ではなく日付から導出**(2026-08-12) | 終了時刻 = `endOfJstDay(max(startsAt, endsAt))`、境界はキャンセル期限と同じ「その日の終わり」。過去分は台帳と合計から外し、ボトムナビ「終了」＝履歴（終わった予約＋キャンセル済み）に畳む。`endsAt` はパーサーが既に抽出済みで台帳だけが捨てていたので受け取るようにする。**コア側の `done` を借りなかった理由**: 導出なら既存レコードが即日正しくなり、移行も cron の自動書き換えも要らず、日付を直せば台帳に戻る。「行った確定」と「決めないまま流れた候補」の区別は既存 `status` で読めるため `done` を足しても新しく分かることはない。コア統合が必要になった時点でこの導出規則が土台になる |

| 12 | Web台帳が保存する時刻は**書き込み時に正規形へそろえる**(2026-08-12) | `startsAt` / `endsAt` はスキーマ上は素の文字列（厳しくすると既存レコードがパースに失敗して一覧から消える）。そのため同じ文字列を読む側で意味が変わり得た: サーバの `Temporal.Instant.from` はオフセット無し `"2026-08-05T10:00"` を拒み時だけの `"+09"` を受け、ブラウザの `Date.parse` はちょうど逆。厳しい側が隠した行を緩い側が数え続け、終わった予約の判定と最大キャンセル料が web と LINE で食い違う。`src/web/instant.ts` で create / patch の両方を1つの正規形に直し、オフセットの無い値は JST として読む（確認メールの「8月5日 10:00」が意味するもの・カレンダーフィードが既に使っていた読み方）。必須の `startsAt` が読めないときは落とさずそのまま残す（落とすと予約がどの一覧にも出ない）。任意の `endsAt` は null にし、`src/parse/validate.ts` が「捨てた」ことを警告する。形式が混在すると `listReservations` の文字列ソートも前後が入れ替わっていたので、並び順も瞬間比較にした |

| 13 | テキスト一次パーサーの**モデルは固定しない**。プロバイダの引退は「読めなかった」と区別してログに出す(2026-08-20) | ADR-5 で選んだ `llama-3.3-70b-versatile` を Groq が引退させ、404 model_not_found を返すようになった(この日、アカウントには Llama 系チャットモデルが1つも無い)。パーサーは決して throw せず失敗を `output: null` にしてチェーンを次へ落とす設計なので、**モデルの消滅は「読めないメール」と同じ見え方**になる — 応答 200・ログ無言・貼り付けは Gemini だけに依存。その Gemini が 503 を返した日に家族の画面へ英語のエラーが出て、初めて発覚した。決定: ①一次は `openai/gpt-oss-120b`(Groq に残る汎用モデルのうち `response_format: json_object` を守る最大のもの。`qwen3.6-27b` は同プロンプトに json_validate_failed を返すため不採用) ②`parserFailures()` / `allParsersFailed()` で「プロバイダが答えなかった」を全経路のログ level に反映(誰も答えなければ error) ③`/api/parse` は `reason` を返し、画面は「読めなかった」と「繋がらなかった」を言い分ける ④起動時に実際のモデル名と鍵の有無をログに出す ⑤5xx だけ1回再試行(429 はクォータなので再試行しない)。**チェーン識別子 `groq-llama` は据え置く**: `parsers.config.json`・`fixtures/parse/`・保存済み ParseJob がこの文字列で attempts を引いており、その記録は実際に Llama が答えた内容だから。**再発防止の残り**: 定期的な疎通確認(1日1回、各プロバイダへ最小リクエスト)は未実装 |

| 14 | **失敗は見た瞬間に記録し、要望はヘルプから受ける。届け先は KV のみ・管理者がマイページで読む**(2026-08-20) | ADR-13 の残課題（定期的な疎通確認が無い）への答え。①クライアントの失敗系は `failed()` 一本に集約し、人が見た瞬間に `POST /api/reports` へ自動記録する（押されなければ届かない設計では、今回と同じ「誰も言わないまま数日」が再発する）。捕まえられなかった例外・rejected promise も1読み込み5件まで記録 ②補足は任意で、既に書かれた記録に `ref` で後から足す ③ヘルプ末尾から `kind:"request"` で要望を受ける。**届いて並ぶだけで、取り入れるかは開発者が読んで決める**（オーナー 2026-08-20） ④cron が1日1回プロバイダに疎通を聞き（`src/parse/canary.ts`）、答えないものを `kind:"system"` として同じ一覧に積む。**チェーンは通さない** — フォールスルーは人にとって正しく、signal としては故障を隠すため ⑤**通知は出さない**（LINE/メール押し出しは不採用・オーナー判断）。見る場所を1つにする ⑥`note` / `sample` は**到着時にサーバで再マスク**（ブラウザ側のマスクを信用しない）。日次上限50件・保存180日。**未解決**: `ParseJob.raw_input` は SDD §10.4 の「ローカル保存・信頼できるアクセス前提」のまま未マスクでマネージド KV に載っている。報告はマスク済みなので、同じ KV に方針の違う2つが並んでいる |

| 15 | **日付だけは規則でも読む — モデルの下の床、モデルへの検査**(2026-09-03) | ADR-13 の再発対策の最後の1つ。`src/parse/rules.ts` が確認メールの書き方（`2026年8月15日(土) 19:00` / `8/20(木) 18:30〜` / `8/1 19時` / チェックイン・アウト / `〜` 範囲、午後・半、全角）を読む。年の無い日付はプロンプトと同じ規則（今日以降で最も近い将来）。①**床**: どのモデルも output を出さなかったテキストで本文が日を名指ししていれば、`parser: "rules"` の attempt として `starts_at`（あれば `ends_at`）を積み、ジョブは failed ではなく **needs_review**（日付入り・店名は人が書く）。モデルが答えている間は参照しない。`allParsersFailed` は rules を数えないので、救われたジョブでも「モデルは全滅」と正しく言える（`/api/parse` は needs_review でも `reason:"unavailable"` を返し、画面は「読み取りにつながらず日時だけ本文から拾った」と言い分ける） ②**検査**: Groq の妥当な答えの `starts_at` が本文に無い日なら review reason `date_not_in_text` を付けて Gemini 査読へ。曜日だけの本文は規則に意見が無い＝不一致ではない。**あえて狭い**: 日付と時刻のみ。店名・場所・金額はモデル。本文に無い日を推測しない。**補助**: `fixtures/parse/*.json` に手で確かめた `truth`（本文だけから決まること。年無しは `starts_at_md`）と `deno task parse:bench <model>`。`expected` は録音であって正解ではない（`real-restaurant-mail` の 29h は誤りで正解は 23h）。規定は正規形（繰り返し料率を畳み・最終境界を捨てる）で比べる |
