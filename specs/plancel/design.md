...
### Web台帳の「終わった予約」（Task 8.2）

Web台帳（`src/web/store.ts`）はコア台帳と違い `done` を持たない。日付が過ぎた予約は
`candidate`/`confirmed` のまま残り、開始日昇順の一覧では**古いものほど先頭に居座る**。
ヘッダの「最大キャンセル料」も、もう起こりえない過去分を合算していた。

**終了は状態ではなく日付から導出する**（`src/web/lifecycle.ts`）:

```
終了時刻 = endOfJstDay(max(startsAt, endsAt ?? 0))
終わった = now > 終了時刻
```

- 境界は**終わる日の終わり（JST 23:59:59.999）**。台帳が既にキャンセル期限で使っている
  「境界はその日の終わり」（`boundaryMs`）と同じ読み方にし、アプリ内に時刻規則を2種類作らない。
- `endsAt` が無ければ開始日。`endsAt < startsAt`（誤パース）は `startsAt` を採る —
  壊れた終了日で未来の予約を隠さないため。
- 遅れる側に倒す（19:00 の食事はその日いっぱい、10:00 チェックアウトも当日いっぱい台帳に残る）。
  進行中の旅行を履歴へ落とす事故が起きない向き。

`endsAt` は新設ではない。パーサーは既に `ends_at`（宿はチェックアウト）を抽出しており
（`src/parse/llm.ts`）、コア `Reservation` にもある。Web台帳だけが捨てていたので、
`webReservationSchema` に `endsAt: string | null`（`.default(null)` で既存レコードも読める）を
足し、パース3経路（`parse-api.ts` / `email-intake.ts` / LINE 登録）で渡す。

表示は、台帳＝これから起こること、履歴＝終わったこと、に分ける:

- `render()` は `cancelled` / `finished` / `active` の3分割。**合計は `active` だけ**で出す。
- ボトムナビ「終了」は履歴（`finished + cancelled` の件数）。中を「終わった予約」「キャンセル済み」の
  2小見出しに分ける。ラベルだけ先に `終了` を名乗って中身がキャンセル済みだったずれを解消する。
- 終了行はカウントダウン帯とキャプションを描かない。`span = max(start - now, DAY)` で満杯に潰れた帯と
  「無料期間は終了」の誤読はこれで消える。ステータスの印は残すので「決めないまま過ぎた」ことは読める。
- LINE も同じ関数で `activeOf` とフッター合計から除外し、`確認` の末尾に「終わった予約 N件」を添える。

**状態を保存しない理由**: 導出なら既存レコードが即日正しくなり、移行も cron の自動書き換えも要らず、
日付を直せば台帳に戻る。「行った確定」と「決めないまま流れた候補」の区別は既存 `status` で読めるので、
`done` を足しても新しく分かることはない。コア統合が必要になった時点で導出規則がそのまま土台になる。

**削除は変えない**（ハード削除1本道のまま）。ただし確認文言は揃える: 台帳の行の
「この予約を削除しますか？」は、同じ不可逆削除なのに「元に戻せない」と言っていない。

**既知のコスト**: 判定規則はクライアント（`web/index.html`）にも同じ1行が必要。日付計算自体は
両側に既にある（`endOfJstDay`）が、`max` との合成は二重化する。**写しは
`src/web/tests/policy_mirror_test.ts` が縛る**（配信HTMLから関数を取り出してサーバと比較。
各ケースをその境界時刻でも判定するので、`>` と `>=` の1ミリ秒のズレも落ちる）。

**保存する時刻は正規形にそろえる**（`src/web/instant.ts`、ADR-12）。`startsAt` / `endsAt` は
スキーマ上は素の文字列（厳しくすると既存レコードが読めず一覧から消える）なので、読む側で
意味が変わり得た: サーバの `Temporal.Instant.from` はオフセット無しを拒み `+09` を受け、
ブラウザの `Date.parse` は逆。create / patch の両方で1つの正規形に直し、オフセットの無い値は
JST として読む（カレンダーフィードが既に使っていた読み方）。必須の `startsAt` が読めない
ときはそのまま残し（落とすと予約がどの一覧にも出ない）、任意の `endsAt` は null にして
`src/parse/validate.ts` が警告を出す。並び順も文字列比較から瞬間比較へ。

**カレンダー**（`src/web/calendar/`）は `endsAt` があればそこまでの予定にする。無い場合と、
開始以前の `endsAt`（誤パース）は従来どおり1時間。終わりが始まりより前の予定は描かずに
拒むカレンダーがあるため。
