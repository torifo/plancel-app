...
- [x] **Task 8.2**: 終わった予約を履歴へ（Web台帳・LINE）（2026-08-12）
  - What: 終了を日付から導出（`endOfJstDay(max(startsAt, endsAt))`）し、台帳と合計から外して
    ボトムナビ「終了」＝履歴（終わった予約＋キャンセル済み）に畳む。`endsAt` はパーサーが既に
    出しているものを台帳が受け取るようにする。状態は保存しない（design.md 参照）
  - Files: `src/web/lifecycle.ts`（新規）, `src/web/store.ts`, `src/web/policy.ts`（`endOfJstDay`
    の export）, `src/web/parse-api.ts`, `src/web/email-intake.ts`, `src/line/web-commands.ts`,
    `web/index.html`
  - Done when: `lifecycle_test.ts`（endsAt 無し/有り/逆転/境界ちょうど/翌0:00）、`endsAt` の往復と
    既存レコードの既定値、LINE の一覧・合計からの除外、パース3経路で `ends_at` が記録に入ること、
    実ブラウザで過去予約が履歴に移り合計から消えることを確認
  - Depends on: Task 8.1
  - レビュー（fable / codex）で出た同種の欠陥もまとめて解消: クライアント写しとサーバの
    不一致（読めない日時）と写しの固定テスト、プラン確定が終わった候補を巻き添えにする件と
    確定件数の表示、保存時刻の正規化（`src/web/instant.ts`。混在形式で並び順も崩れていた）、
    カレンダーが `endsAt` を無視して1時間の予定にする件、共有キャンセル済みがどこにも
    出ない件、デモ台帳が21日後に空で開く件、終了日フォームの日比較
